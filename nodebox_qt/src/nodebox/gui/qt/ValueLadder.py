from PyQt4.QtGui import QApplication, QTextCursor, QToolTip
...
class ValueLadder:
    ...
    def __init__(self, textView, value, clickPos, screenPoint, viewPoint):
        ...
        self.origX, self.origY = screenPoint.x(), screenPoint.y()
        self.x, self.y = screenPoint.x(), screenPoint.y()
        self.viewPoint = self.textView.mapToGlobal(viewPoint)
        ...
        success, output = self.textView._document._boxedRun(self._parseAndCompile)
        if success:
            self.show()
        else:
            self.textView._document._flushOutput(output)
        
    def _parseAndCompile(self):
        from compiler import parse
        ast = parse(self.patchedSource.encode('ascii', 'replace') + "\n\n")
        self._checkSigns(ast)
        self.textView._document._compileScript(self.patchedSource)
    ...
    def hide(self):
        ...
        cursor.setPosition(range[0], QTextCursor.MoveAnchor)
        cursor.movePosition(QTextCursor.NextCharacter, QTextCursor.KeepAnchor, range[1])
        cursor.removeSelectedText()
        cursor.insertText(value)

    def mouseDragged_(self, event):
        mod = int(QApplication.keyboardModifiers())
        newX, newY = event.pos().x(), event.pos().y()
        deltaX = newX-self.x
        delta = deltaX
        if self.negative:
            delta = -delta
        if mod & Qt.AltModifier > 0:
            delta /= 100.0
        elif mod & Qt.ShiftModifier > 0:
            delta *= 10.0
        self.value = self.type(self.value + delta)
        self.x, self.y = newX, newY
        self.dirty = True
        self.textView._document.magicvar = self.value
        self.textView._document.currentView.direct = True
        self.textView._document.runScriptFast_()
        self.draw()